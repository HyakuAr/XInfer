...
void rope(sycl::queue& q,
    ...
    if (q_ptr && num_q_heads > 0) {
        int64_t total_pairs = num_tokens * num_q_heads * half_dim;
        q.parallel_for(sycl::range<1>(static_cast<size_t>(total_pairs)), [=](sycl::id<1> idx) {
            int64_t global_idx = idx[0];
            int64_t pair_idx = global_idx % half_dim;
            int64_t rem = global_idx / half_dim;
            int64_t head_idx = rem % num_q_heads;
            int64_t token_idx = rem / num_q_heads;

            int64_t pos = positions[token_idx];
            float freq = 1.0f / sycl::pow(theta, static_cast<float>(2 * pair_idx) / static_cast<float>(head_dim));
            ...
            int64_t idx0 = base_offset + 2 * pair_idx;
            int64_t idx1 = idx0 + 1;
            ...
            q_ptr[idx1] = v0 * sin_val + v1 * cos_val;
        });
    }

    // Apply to K heads
    if (k_ptr && num_kv_heads > 0) {
        int64_t total_pairs = num_tokens * num_kv_heads * half_dim;
        q.parallel_for(sycl::range<1>(static_cast<size_t>(total_pairs)), [=](sycl::id<1> idx) {
            int64_t global_idx = idx[0];
            int64_t pair_idx = global_idx % half_dim;
            int64_t rem = global_idx / half_dim;
            ...
            k_ptr[idx1] = v0 * sin_val + v1 * cos_val;
        });
    }
