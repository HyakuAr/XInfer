...
sycl::event rope(sycl::queue& q,
    ...
    return q.parallel_for(sycl::range<2>(total_heads, static_cast<size_t>(half_rot)), [=](sycl::id<2> idx) {
        ...
        float* target_ptr = is_q ? q_ptr : k_ptr;
        ...
        float v0 = target_ptr[idx0];
        float v1 = target_ptr[idx1];

        target_ptr[idx0] = v0 * cos_val - v1 * sin_val;
        target_ptr[idx1] = v1 * cos_val + v0 * sin_val;
    });
}
